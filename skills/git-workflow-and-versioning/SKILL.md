...
- **WIP limit under review — fixes before new claims.** When operating in a reviewed loop, start every wake/claim by **enumerating open `[CHANGES-REQUESTED]` entries naming your branches**; if two or more are pending, the next unit of work is a fix pass, not a new claim. A priority *ordering* in a standing prompt is not a *limit* — agents drift toward novel work over rework, and unconsumed fix passes compound (stale branches, repeated gate runs into known failures, re-review churn). (Audited: two fix passes sat pending while two new design folds were claimed instead.)
