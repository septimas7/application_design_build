...
- A harness or evidence script whose test filter can match **zero tests and still exit green** — runners exit 0 on zero matches, so a rename silently unwires the suite. Scripts assert a nonzero pass count (see `running-quality-gates`); renaming a cited test means grepping for every place the old name is pinned.
